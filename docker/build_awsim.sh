...
set -x
docker buildx bake --no-cache --load --progress=plain -f "$SCRIPT_DIR/awsim/docker-bake.hcl" \
    ...
    --set "devel.tags=shmpwk:$rosdistro-latest$image_name_suffix" \
    # --set "prebuilt.tags=shmpwk:$rosdistro-latest-prebuilt$image_name_suffix" \
    "${targets[@]}"
set +x
